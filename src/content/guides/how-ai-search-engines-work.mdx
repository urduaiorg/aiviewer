...
tags: ["ai-search", "perplexity", "rag", "explainer"]
draft: false
---
...
This shift is driven entirely by a technology called **RAG (Retrieval-Augmented Generation)**. For a deeper technical breakdown of RAG and when to use it, see our guide on [fine-tuning vs RAG](/guides/fine-tuning-vs-rag/). Here is exactly how Answer Engines like Perplexity and Google's SGE (Search Generative Experience) actually work under the hood.

## The Secret Sauce: RAG

If you ask a standard AI model (like [ChatGPT](/tools/chatgpt/)) an obscure question about yesterday's news, it will fail. AI models are trained on massive datasets that have a strict "cut-off date." They do not natively know the live internet.

**RAG is the solution.**

When you type a query into an AI Search Engine like [Perplexity](/tools/perplexity-ai/), here is the chronological pipeline that executes in the ~1.5 seconds before your answer appears:
...
The Answer Engine takes all those extracted facts and feeds them into its core LLM (often a proprietary model or a tuned version of [Claude](/tools/claude-ai/) 3.5). The prompt to the LLM essentially says: 
...
- **You.com:** Offers a chat-based search experience with the ability to switch between different AI models (GPT-4o, Claude, Llama). For a head-to-head comparison of these underlying models, see our [ChatGPT vs Claude breakdown](/guides/chatgpt-vs-claude-2026/). Its "Research" mode generates comprehensive reports with citations, aimed at academic and professional users.
